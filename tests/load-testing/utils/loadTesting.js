...
const startLoadTesting = (configFile, localSchema) => {
  ...
  if (!process.env.ACCESS_TOKEN) {
    ...
    throw 'Error: No Access Token. Set ACCESS_TOKEN as env variable.';
  }

  const schemaPath = artilleryConfigPath('schema.gql');

  const testName = name || url;
...
const runLoadTesting = (configFile, spinner) => {
  ...
  if (configFile.config.queryFile) {
    configFile.config['queryFilePath'] = path.join(
      path.resolve(), 'tests/load-testing/outputs');
  }
  ...
  fs.writeJSON(newConfigFile, configFile, (err) => {
    ...
    const { config: {
      ...
      url, duration = 5, arrivalRate = 10, withOutput, queryFilePath, headers,
    } } = require(newConfigFile);
    let reportPath;

    const artilleryBin = path.join(
      path.resolve(), 'node_modules/.bin/artillery');
    let options = [
      'run',
      '--target',
      `${url}`,
    ...
    // TODO: Fix this
    // Optionally override the config values from the artillery.yml
    // Not working on Windows due to command line error when passing in JSON
    ...
    //   ['--overrides', JSON.stringify(configOverride)]);

    if (withOutput) {
      const date = moment().format('YYYYMMDDHHMMSS').toString();
      reportPath = path.join(
        path.resolve(), 'tests/load-testing/outputs', `${date}.json`);
    ...
    artilleryRun.on('exit', code => {
      if (code === 0) {
        if (withOutput && reportPath) {
          console.log(
            `Full report run: "npx artillery report ${reportPath}"`, '\n');
        }
        if (queryFilePath) {
          console.log(
            `Query file: ${queryFilePath}/easygraphql-load-tester-queries.json`,
            '\n');
        }
...
const artilleryConfigPath = (fileName = '') => path.join(
  __dirname, '.', 'artilleryConfig', fileName);
